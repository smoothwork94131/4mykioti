    ...
    public function datatables($status)
    {
        ...
        return Datatables::of($datas)
            ->addColumn('name', function (Verification $data) {
                ...
                if($data->type == 0)
                    $name = isset($data->user->name) ? '<a href="' . route('admin-user-show', $data->user->id) . '" target="_blank">' . $data->user->name . '</a>' : 'Removed';    
                return $name;
            })
            ...
            ->editColumn('type', function (Verification $data) {
                
                if($data->type == 1) {
                    return "<span style='color: white; padding-left: 5px; padding-right: 5px; background-color: blue'>Seller</span>";
                } else {
                    return "<span style='color: white; padding-left: 5px; padding-right: 5px; background-color: green'>Buyer</span>";
                }
