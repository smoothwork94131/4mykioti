...
class SubCategoryController extends Controller
{
    ...
    public function datatables()
    {
        ...
        return Datatables::of($datas)
            ...
            ->addColumn('cod', function (Subcategory $data) {
                $class = $data->cod == 1 ? 'drop-success' : 'drop-danger';
                $s = $data->cod == 1 ? 'selected' : '';
                $ns = !$data->cod ? 'selected' : '';
                return '<div class="action-list"><select class="process select droplinks ' . $class . '"><option data-val="1" value="' . route('admin-subcat-cod', ['id1' => $data->id, 'id2' => 1]) . '" ' . $s . '>Activated</option><<option data-val="0" value="' . route('admin-subcat-cod', ['id1' => $data->id, 'id2' => 0]) . '" ' . $ns . '>Deactivated</option>/select></div>';
            })
