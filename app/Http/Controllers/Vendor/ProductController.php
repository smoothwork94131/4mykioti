...
class ProductController extends Controller
{
         ...
         return Datatables::of($datas)
            ...
            ->addColumn('action', function(Product $data) {
                return '<div class="action-list">
                    <a href="' . route('vendor-prod-edit',$data->id) . '"><i class="fas fa-edit"></i>'.$this->vendor_language->lang715.'</a>
                    <a href="javascript" class="set-gallery" data-toggle="modal" data-target="#setgallery">
                        <input type="hidden" value="'.$data->id.'"><i class="fas fa-eye"></i> '.$this->vendor_language->lang716.'
                    </a>
                    <a href="javascript:;" data-href="' . route('vendor-prod-delete',$data->id) . '" data-toggle="modal" data-target="#confirm-delete" class="delete">
    ...
    public function createPhysical()
    {
        ...
        $cats = Category::all();
        $sign = Currency::where('is_default','=',1)->first();
        $locs = $user->locations()->orderBy('id','desc')->get();
        return view('vendor.product.create.physical',compact('cats','sign', 'locs'));
    }
    ...
    public function uploadUpdate(Request $request,$id)
    {
        ...
        $hasTransparency = $this->hasTransparency(imagecreatefrompng(public_path() . '/assets/images/products/' . $image_name));

        if(!$hasTransparency){
            try {

                $removebg->file(public_path() . '/assets/images/products/' . $image_name)
                ->headers([
                    'X-Width' => 600,
                    'X-Height' => 600,
                ])
                ...
                ->save(public_path() . '/assets/images/products/' . $image_name);
        ...
        $hasTransparency = $this->hasTransparency(imagecreatefrompng(public_path() . '/assets/images/thumbnails/' . $thumbnail));
        ...
        if(1)
        {
            ...
            {      
                ...
                if($input['strain-feature-photo']) {
                    ...
                    $destination = public_path().'/assets/images/products/vendor/'.$user->id.'/'.$name;
                    if(!file_exists(public_path().'/assets/images/products/vendor/'.$user->id))
                        mkdir(public_path().'/assets/images/products/vendor/'.$user->id, 0777, true);
                    

                    copy($source, $destination);
                } else {
                    $file = $request->file('photo');
                    $file->move('assets/images/products/vendor/'.$user->id.'/',$name);
                }

                $input['photo'] = 'vendor/'.$user->id.'/'.$name;
            } else {
                $input['photo'] = "";
            }
            ...
            if($prod->photo) {
                ...
                $thumbnail = "product-vendor-".$user->id."-".$thumbnail;
            ...
            if ($files = $request->file('gallery')){
                foreach ($files as  $key => $file){
                    if(in_array($key, $request->galval))
                    {
                        ...
                        $hasTransparency = $this->hasTransparency(imagecreatefrompng($gallery_path));
    ...
    public function edit($id)
    {
        ...
        elseif($data->type == 'License')
            ...
            return view('vendor.product.edit.physical',compact('cats','data','sign', 'locs'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        {              
            ...
            $file->move('assets/images/products/vendor/'.$data->user_id.'/',$name);

            $input['photo'] = 'vendor/'.$data->user_id.'/'.$name;
        } 
        //Check Types
        ...
        if ($file = $request->file('photo')) {
            ...
            // $check_image = new \Imagick();
            // $check_image->readImage($newimg_path);
            // $hasTransparency = $check_image->getImageAlphaChannel();
    ...
    public function catalogupdate(Request $request, $id)
    {
        ...
        //--- Logic Section
            ...
            if($request->is_photo == '1')
            {
                ...
                {      
                    ...
                    $path = public_path() . 'assets/images/products/' . $name;

                    $hasTransparency = $this->hasTransparency(imagecreatefrompng($path));
        ...
        $newimg = Image::make(public_path().'/assets/images/products/'.$prod->photo)->resize(800, 800);
        $photo = time().str_random(8).'.png';
        $newimg->save(public_path().'/assets/images/products/'.$photo);
        $newimg_path = public_path().'/assets/images/products/'.$photo;
    ...
    public function category_license(Request $request) {
        $category_id = $request->category_id;
        $data = Category::findOrFail($category_id);
        $skip_license = $data->skip_license;
        echo $skip_license;
    }
}