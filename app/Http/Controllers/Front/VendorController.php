...
class VendorController extends Controller
{
    ...
    public function index(Request $request, $slug, $slug1 = null, $slug2 = null, $slug3 = null)
    {
        $this->code_image();
        // $sort = "";
        ...
        $prods = Product::when($minprice, function ($query, $minprice) {
            ...
            ->when(empty($sort), function ($query, $sort) {
                return $query->orderBy('id', 'DESC');
            })->where('status', 1)->where('user_id', $vendor->id);
            ...
            if(!empty($cat)){
                $prods = $prods->where('category_id', $cat->id);
            }

            if(!empty($subcat)){
                $prods = $prods->where('subcategory_id', $subcat->id);
            }

            if(!empty($childcat)){
                $prods = $prods->where('childcategory_id', $childcat->id);
            }
        ...
        $vprods = (new Collection($prods))->paginate(9);
        $data['vprods'] = $vprods;
