(function($) {
    ...
    $(document).ready(function() {
        ...
        // Check Click1 :)
        $(document).on(".checkclick1", "change", function() {
            if (this.checked) {
    ...
    $("#whole-btn").on('click', function() {
        ...
        if (whole_sell > $("[name='whole_sell_qty[]']").length) {
            $("#whole-section").append('' +
                ...
                '</div>' +
                '<div class="col-lg-6">' +
                '<input type="number" step="0.01" name="whole_sell_discount[]" class="input-field" placeholder="' + langg.lang662 + '" min="0" required>' +
                '</div>' +
                '</div>' +
                '</div>' +
                '');
