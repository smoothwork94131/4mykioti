...
class GeneralSettingController extends Controller
{
    ...
    public function generalupdate(Request $request)
    {
        ...
        //--- Logic Section
            ...
            if ($file = $request->file('prodimage')) {
                $name = time() . $file->getClientOriginalName();
                $name = str_replace(array( '\'', '"', ',' , ';', '<', '>', '!', '@', '#', '$', '%', '^', '&', '*', ':' ), '', $name);
                $data->upload_prod_image($name, $file, $data->prod_image);
                $input['prod_image'] = $name;
            }
    ...
    {
        return view('admin.generalsetting.prodimage');
    }
    ...
    public function solo() {
        $cats = Category::all();
        return view('admin.generalsetting.solo', compact('cats')); 
    }

    public function solo_mode($status) {
        $data = Generalsetting::findOrFail(1);
        $data->solo_mode = $status;
        $data->update();
    }

    public function solo_save(Request $request) {
        $category = $request->category;

        $data = Generalsetting::findOrFail(1);
        $data->solo_category = $category;
        $data->solo_mode = $request->solo_mode;
        $data->update();
    ...
    public function agepopup() {
        $cats = Category::all();
        return view('admin.generalsetting.agepopup', compact('cats')); 
    }

    public function active_agepopup($status) {
        $data = Generalsetting::findOrFail(1);
        $data->age_checker = $status;
        $data->update();
    }

    public function save_agepopup(Request $request) {
        $text = $request->agepopup_text;

        $data = Generalsetting::findOrFail(1);
        $data->agepopup_text = $text;
        $data->update();
    ...
    public function slider_location($status) 
    {
        $data = Generalsetting::findOrFail(1);
        $data->banner_product_location = $status;
        $data->update();
    }
