...
class ProductController extends Controller
{
    ...
    public function uploadUpdate(Request $request, $id)
    {
        ...
        $image_name = $id.'_logo.png';

        $apiKey = "YAypVmKK55sfxF4SPZdMFLyx";
        $removebg = new RemoveBg($apiKey);

        $main_path = public_path() .'/assets/images/products/admin/main/'.$image_name;
        file_put_contents($main_path, $image);

        $removebg->file($main_path)
        ->headers([
            'X-Width' => 600,
            'X-Height' => 600,
        ])
        ...
        $input['photo'] = 'admin/main/'.$image_name;
        $data->update($input);

        if ($data->thumbnail != null) {
            if (file_exists(public_path() . '/assets/images/products/'.$data->thumbnail)) {
                unlink(public_path() . '/assets/images/products/'.$data->thumbnail);
            }
        }

        $img = Image::make(public_path() . '/assets/images/products/'.$data->photo)->resize(285, 285);

        $thumbnail = $id.'_logo.png';
        $thumbnail_path = public_path() . '/assets/images/products/admin/thumbnails/'. $thumbnail;

        $img->save($thumbnail_path);

        $removebg->file($thumbnail_path)
        ->headers([
            'X-Width' => 600,
            'X-Height' => 600,
        ])
        ...
        $data->thumbnail = 'admin/thumbnails/'.$thumbnail;
        $data->update();
    ...
    public function store(Request $request)
    {
        ...
        $total_record = Product::orderBy('id', 'desc')->get();
        ...
        list(, $image) = explode(',', $image);
        $image = base64_decode($image);
        $image_name = $last_id.'_logo.png';
        
        $apiKey = "YAypVmKK55sfxF4SPZdMFLyx";
        $removebg = new RemoveBg($apiKey);

        $main_directory = public_path() . '/assets/images/products/admin/main';

        if (!File::isDirectory($main_directory)) {
            File::makeDirectory($main_directory, 0777, true, true);
        }
        ...
        $thumbnail = $last_id.'_logo.png';

        $thumbnail_directory = public_path() . '/assets/images/products/admin/thumbnails';

        if (!File::isDirectory($thumbnail_directory)) {
            File::makeDirectory($thumbnail_directory, 0777, true, true);
        }
        ...
        $prod->thumbnail = 'admin/thumbnails/'.$thumbnail;
        $prod->update();

        // Add To Gallery If any
        $lastid = $data->id;

        $gallery_directory = public_path() . '/assets/images/products/admin/gallery/'. $lastid;

        if (!File::isDirectory($gallery_directory)) {
            File::makeDirectory($gallery_directory, 0777, true, true);
        }
        ...
        if ($files = $request->file('gallery')) {
            foreach ($files as $key => $file) {
                if (in_array($key, $request->galval)) {
                    ...
                    $gallery = new Gallery;
                    $gallery_total_record = Gallery::orderBy('id', 'desc')->get();

                    $gallery_last_id = 1;
                    if($gallery_total_record->count() != 0) {
                        $gallery_last_record = Gallery::orderBy('id', 'desc')->first();
                        $gallery_last_id = $gallery_last_record->id;
                        $gallery_last_id++;
                    }

                    $name = $lastid.'_'.$gallery_last_id.'.png';

                    $gallery_path = public_path() . '/assets/images/products/admin/gallery/'.$lastid.'/'.$name;
                    $file->move('assets/images/products/admin/gallery/'.$lastid, $name);
                    ...
                    $gallery['photo'] = 'admin/gallery/'.$lastid.'/'. $name;
    ...
    public function destroy($id)
    {
        ...
        if ($data->galleries->count() > 0) {
            foreach ($data->galleries as $gal) {
                if (file_exists(public_path() . '/assets/images/products/' . $gal->photo)) {
                    unlink(public_path() . '/assets/images/products/' . $gal->photo);
                }
                $gal->delete();
            }
        ...
        if (file_exists(public_path() . '/assets/images/products/' . $data->thumbnail) && $data->thumbnail != "") {
            unlink(public_path() . '/assets/images/products/' . $data->thumbnail);
        }
