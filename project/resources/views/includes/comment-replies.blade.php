            ...
            @foreach($productt->comments()->orderBy('created_at','desc')->get() as $comment)
                <li>
                    <div class="single-comment comment-section">
                        <div class="left-area">
                            <img src="{{$comment->user->photo != null ? asset('assets/images/users/'.$comment->user->photo) : asset('assets/images/noimage.png')}}"
                                 alt="">
                                ...
                                <div class="links">
                                    ...
                                    @if(count($comment->replies) > 0)
                                        <a href="javascript:;" class="comment-link view-reply mr-2"><i
                                                    class="fas fa-eye "></i>{{ $langg->lang108 }} {{ count($comment->replies) == 1 ? $langg->lang109 : $langg->lang110  }}
                                        </a>
                                    @endif
                                    @if(Auth::guard('web')->user()->id == $comment->user->id)
                                        ...
                                        <a href="javascript:;"
                                           ...
                                           class="comment-link comment-delete mr-2"><i
                                                    class="fas fa-trash"></i>{{ $langg->lang112 }}</a>
                                    @endif
                                </div>
                            </div>
                        </div>
                    ...
                    @if($comment->replies)
                        @foreach($comment->replies as $reply)
                            <div class="single-comment replay-review hidden">
                                <div class="left-area">
                                    <img src="{{ $reply->user->photo != null ? asset('assets/images/users/'.$reply->user->photo) : asset('assets/images/noimage.png') }}"
                                         alt="">
