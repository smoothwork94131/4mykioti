...
@section('content')
    <div class="content-area">
        <div class="mr-breadcrumb">
            <div class="row">
                <div class="col-lg-12">
                    <h4 class="heading">{{ __("Product Policy") }}</h4>
                    <ul class="links">
                        <li>
                        ...
                        <li>
                            <a href="javascript;">{{ __("Products Policy") }}</a>
                        </li>
                    </ul>
                </div>
            </div>
...
@section('scripts')
    ...
    <script type="text/javascript">
        ...
        var table = $('#geniustable').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: '{{ route('admin-prod-policy-datatables') }}',
            columns: [
                {data: 'name', name: 'name'},
        ...
        $(function () {
            $(".btn-area").append(
                '<div class="col-sm-4 table-contents">' +
                    '<a class="add-btn" href="{{route('admin-prod-policy-create')}}">' +
                    '<i class="fas fa-plus"></i> <span class="remove-mobile">{{ __("Add New Product Policy") }}<span>' +
                    '</a>' +
                '</div>'
            );
        });
